...
func (c *Campaign) Validate() error {
	...
	// All campaigns require template, page, and SMTP
	switch {
	...
	// Additional validation for calendar campaigns
	if c.CampaignType == "calendar" {
		switch {
		case c.EventStartTime.IsZero():
			return errors.New("Event start time is required for calendar campaigns")
		case c.EventDuration <= 0:
			return errors.New("Event duration must be greater than 0 for calendar campaigns")
		case c.OrganizerName == "":
			return errors.New("Organizer name is required for calendar campaigns")
		}
	}
...
func PostCampaign(c *Campaign, uid int64) error {
	// Set default campaign type if not specified
	if c.CampaignType == "" {
