...
// Toggle campaign type (email vs calendar)
function toggleCampaignType() {
    var campaign_type = $("#campaign_type").val();
    
    if (campaign_type === "calendar") {
        // Keep email template visible, hide test email button
        $("#sendTestEmailContainer").hide();
        
        // Show calendar-specific elements
        $("#calendar_section").show();
    } else {
        // Show test email button
        $("#sendTestEmailContainer").show();
    ...
    Swal.fire({
        ...
        preConfirm: function () {
            return new Promise(function (resolve, reject) {
                ...
                // Get campaign type
                var campaign_type = $("#campaign_type").val() || "email";
                
                // Build base campaign object
                campaign = {
                    name: $("#name").val(),
                    url: $("#url").val(),
                ...
                // Add email-specific fields if email campaign
                ...
                // Add calendar-specific fields if calendar campaign
                else if (campaign_type === "calendar") {
                    campaign.template = {
                        name: $("#template").select2("data")[0].text
                    };
                    campaign.page = {
                    ...
                    campaign.platform_type = $("#platform_type").val();
                    campaign.event_meeting_url = $("#event_meeting_url").val();
                    campaign.event_start_time = moment($("#event_start_time").val(), "MMMM Do YYYY, h:mm a").utc().format();
                    campaign.event_duration = parseInt($("#event_duration").val()) || 60;
                    campaign.organizer_name = $("#organizer_name").val();
                    campaign.organizer_email = $("#organizer_email").val();
                }
...
window.dismiss = function() {
    ...
    $("#event_meeting_url").val("");
    $("#event_start_time").val("");
    $("#event_duration").val("60");
    $("#organizer_name").val("");
    $("#organizer_email").val("");
    // Reset to email view - calendar section hidden, test button visible
    $("#calendar_section").hide();
    $("#sendTestEmailContainer").show();
    $("#modal").modal('hide');
}
...
function setupOptions() {
    ...
    if ($("#template").hasClass("select2-hidden-accessible")) {
        $("#template").select2('destroy');
    }
    if ($("#page").hasClass("select2-hidden-accessible")) {
        $("#page").select2('destroy');
    }
